 ...
 * - Uses Azure Grok-4-fast-reasoning to generate full nutrition profile
...
export default function CustomDistrictSearch({ onDistrictAdded }) {
  ...
  const modalContent = isOpen ? (
    <div style={{
      ...
      background: 'rgba(3,6,10,0.82)', backdropFilter: 'blur(4px)',
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
            <button
              className="btn"
              onClick={() => setIsOpen(false)}
              style={{ flex: 1, background: 'rgba(255,255,255,0.06)', color: 'var(--text-secondary)', border: '1px solid var(--border)', padding: '10px 16px', borderRadius: 8, cursor: 'pointer' }}
            >
              Cancel
            </button>
            <button
              ...
              style={{
                flex: 2,
                opacity: (!selected || analyzing) ? 0.5 : 1,
                padding: '10px 16px',
                borderRadius: 8,
                ...
                fontWeight: 600,
                border: 'none',
                cursor: (!selected || analyzing) ? 'not-allowed' : 'pointer'
              }}
            >
